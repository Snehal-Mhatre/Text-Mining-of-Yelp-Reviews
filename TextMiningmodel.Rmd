...
ggplot(resReviewsData, aes(x=stars)) + geom_bar(width = 0.4, fill = "steelblue") + xlab("Stars") + ylab("Number of Reviews")
...
line_graph <- resReviewsData %>% group_by(stars) %>% summarize(mean(funny))
ggplot(line_graph) + aes(x=stars, y=line_graph$`mean(funny)`, fill=line_graph$stars) + geom_line() + xlab("Stars") + ylab("Average of Funny Comments")

#Plot showing relation of Star ratings to word ‘cool’
ggplot(resReviewsData, aes(x= cool, y=stars)) +geom_point()
line_graph1 <- resReviewsData %>% group_by(stars) %>% summarize(mean(cool))
ggplot(line_graph1) + aes(x=line_graph1$stars, y=line_graph1$`mean(cool)`, fill=line_graph1$stars) + geom_line() + xlab("Stars") + ylab("Average of Cool Comments")

#Plot showing relation of Star ratings to word ‘useful’
ggplot(resReviewsData, aes(x= useful, y=stars)) +geom_point()
line_graph2 <- resReviewsData %>% group_by(stars) %>% summarize(mean(useful))
ggplot(line_graph2) + aes(x=line_graph2$stars, y=line_graph2$`mean(useful)`, fill=line_graph2$stars) + geom_line() + xlab("Stars") + ylab("Average of Useful Reaction")
...
#Sentiment analysis using the 3 sentiment dictionaries available with tidytext (use library(textdata))
#Matching Terms for each Dictionary - Answer (C) - Part 1
...
#considering reviews with 1 & 2 stars as negative, and this with 4 & 5 stars as positive
...
# Dictionary 2 - "nrc" - Review Sentiment Analysis
```{r}
#with "nrc" dictionary
...
#how do the dictionaries compare in terms of predictive performance ? Then with a combination of
#the three dictionaries, ie. combine all dictionary terms.
...
#we can learn a model to predict hiLo ratings, from words in reviews
```{r message =FALSE, cache=TRUE}
...
#create Document Term Matrix
revDTM_sentiComboDict <- rrSenti_ComboDict %>%  pivot_wider(id_cols = c(review_id,stars), names_from = word, values_from = tf_idf)  %>% ungroup()
...
#filter out the reviews with stars=3
#calculate hiLo sentiment(1 is assigned to 4 and 5/-1 is assigned to 1 and 2)
revDTM_sentiComboDict <- revDTM_sentiComboDict %>% filter(stars!=3) %>% mutate(hiLo=ifelse(stars<=2, -1, 1)) %>% select(-stars)
